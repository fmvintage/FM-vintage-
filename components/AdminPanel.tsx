...
const AdminPanel: React.FC<AdminPanelProps> = ({ 
  ...
  const getStatusBadgeClass = (status: string) => {
    switch (status) {
      ...
      case 'Packed': return 'bg-indigo-500 text-white';
                        ...
                        {order.status !== 'Delivered' && order.status !== 'Cancelled' && (
                          <>
                            {order.status === 'Pending' && (
                              <button onClick={() => onUpdateOrderStatus(order.id, 'Packed')} className="flex-1 min-w-[80px] bg-indigo-600 text-white py-2 font-black uppercase text-[8px] rounded-sm">Pack Item</button>
                            )}
                            {order.status === 'Packed' && (
                              <button onClick={() => onUpdateOrderStatus(order.id, 'Shipped')} className="flex-1 min-w-[80px] bg-blue-600 text-white py-2 font-black uppercase text-[8px] rounded-sm">Ship Item</button>
                            )}
                            {order.status === 'Shipped' && (
                              <button onClick={() => onUpdateOrderStatus(order.id, 'Delivered')} className="flex-1 min-w-[80px] bg-green-600 text-white py-2 font-black uppercase text-[8px] rounded-sm">Deliver</button>
                            )}
                            <button onClick={() => onUpdateOrderStatus(order.id, 'Cancelled')} className="flex-1 min-w-[80px] bg-red-600 text-white py-2 font-black uppercase text-[8px] rounded-sm">Cancel</button>
                          </>
                        )}
                     </div>
                  </div>
